...
void isr_handler(uint64_t isr_num, uint64_t error_code, uint64_t rip) {
    ...
    serial_puts(": ");
    if (isr_num < 32) serial_puts(exception_names[isr_num]);
    else serial_puts("Desconocida");
    serial_puts(" | Error code: "); serial_hex(error_code);
    serial_puts(" | RIP: "); serial_hex(rip);
    serial_puts("\n[!] Sistema detenido.\n");
